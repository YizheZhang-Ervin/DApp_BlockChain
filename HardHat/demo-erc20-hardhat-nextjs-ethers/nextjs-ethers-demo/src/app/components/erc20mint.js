...
// 导入 MyCoin 智能合约的 ABI 文件，用于与合约进行交互
  ...
  async function handleMint() {
    if (window.ethereum) {
      ...
      try {
        ...
        const response = await contract.mint(ethers.BigNumber.from(mintAmountInETH));
    ...
    <>
      ...
      <div className="flex flex-col flex-grow justify-center items-center font-wq mb-12 mt-20 text-white">
        <div className="w-[640-px] text-center">
          {/* 标题：铸造 MyCoin */}
          <h1 className="text-6xl text-[#fff]">铸造 MyCoin</h1>
          {isConnected ? (
            // 如果已连接钱包
            <>
              <p className="text-4xl mt-20 mb-12 animate-pulse">
                开始铸造你的第一个 MyCoin 代币吧!
              </p>
              {/* 输入框：输入用户想铸造的代币数量 */}
              ...
              <div className="flex-col justify-center items-center mt-8">
                ...
                {/* 显示当前用户的代币余额 */}
                <p className="t text-[#fff] text-xl animate-pulse mt-4">
                  当前的 MyCoin 代币余额：{" "}
